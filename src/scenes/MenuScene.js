...
export class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Ocean background
    this.add.rectangle(0, 0, W, H, 0x0a1628).setOrigin(0, 0);

    // Animated wave tiles
    this.waves = [];
    for (let i = 0; i < 20; i++) {
      const wx = Phaser.Math.Between(0, W);
      const wy = Phaser.Math.Between(0, H);
    ...
    // Title
    this.add.text(W / 2, H * 0.18, 'BLACK TIDE', {
    ...
    this.add.text(W / 2, H * 0.18 + Math.floor(W / 10), 'TRADER', {
    ...
    this.makeButton(W / 2, btnY, btnW, btnH, 'NEW GAME', 0x1a6e2e, () => {
      this.registry.set('player', JSON.parse(JSON.stringify(INITIAL_PLAYER)));
      this.scene.start('GameScene');
    });

    if (hasSave()) {
      this.makeButton(W / 2, btnY + gap, btnW, btnH, 'CONTINUE', 0x1a3a6e, () => {
        this.scene.start('GameScene', { load: true });
      });

      this.makeButton(W / 2, btnY + gap * 2, btnW, btnH, 'DELETE SAVE', 0x6e1a1a, () => {
        deleteSave();
        this.scene.restart();
      });
    }

    this.add.text(W / 2, H - 20, 'Use WASD / Arrow Keys or tap to move  •  Tap port to dock', {
      fontSize: '11px',
      fill: '#4a7aaa',
      fontFamily: 'Courier New',
    }).setOrigin(0.5, 1);
  ...
  makeButton(x, y, w, h, label, color, callback) {
    const bg = this.add.rectangle(x, y, w, h, color, 1).setOrigin(0.5).setInteractive({ useHandCursor: true });
